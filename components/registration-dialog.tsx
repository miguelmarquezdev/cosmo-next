...
export function RegistrationDialog({label="Comienza gratis",className,variant="default",autoOpen=false,notice}:{label?:string;className?:string;variant?:"default"|"outline"|"ghost";autoOpen?:boolean;notice?:string}){
 ...
 return <Dialog open={open} onOpenChange={reset}><DialogTrigger asChild><Button variant={variant} className={className}>{label}{variant!=="ghost"&&<ArrowRight className="size-4"/>}</Button></DialogTrigger><DialogContent className="sm:max-w-[620px]">
  ...
  {notice&&step===1&&<div className="rounded-xl border border-primary/20 bg-primary/[.07] px-4 py-3 text-sm leading-6 text-foreground">{notice}</div>}
