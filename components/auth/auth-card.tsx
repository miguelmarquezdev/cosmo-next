...
export function AuthCard({mode}:{mode:"login"|"register"}){
 ...
 async function google(){try{setBusy(true);const s=createClient();const next=params.get("next")||"/cliente";const{error}=await s.auth.signInWithOAuth({provider:"google",options:{redirectTo:`${location.origin}/auth/callback?next=${next}`,queryParams:{prompt:"select_account",access_type:"offline"}}});if(error)throw error}catch(e:any){toast.error(e.message||"No se pudo conectar con Google");setBusy(false)}}
